...
class TaskModel extends Model
{
    ...
    protected $allowedFields    = ['Taskid', 'Periode_id', 'Location_id', 'Sublocation_id', 'Approval1', 'ApprovalDate1', 'Approval2', 'ApprovalDate2', 'Equipment_id', 'Type_id', 'TaskType', 'NStart', 'NEnd', 'Priority', 'Status', 'Created', 'AssignTo', 'Duedate', 'Duration', 'Shift', 'Notes', 'Completed'];
    ...
    public function taskcmopen($role = null)
    {
        ...
        $uid = session()->get('uid');
        if ($role == 'Supervisor') {
            $sql = 'select t.Taskid,e.Equipmentname,t.Notes,t.Created from task t inner join equipment e on e.Equipmentid=t.Equipment_id  where t.status=1 and t.TaskType=3';
        } elseif ($role == 'Manager') {
    ...
    public function get_work_openua()
    {
        $uid = session()->get('uid');
        $db = \Config\Database::connect();
        $sql = "select*from task t inner join equipment e on e.Equipmentid=t.Equipment_id inner join periodpm p on p.Periodid=t.Periode_id inner join
